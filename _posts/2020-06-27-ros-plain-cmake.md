...
In the CMakeLists.txt of the catkin package we can use the `find_package` mechanism to include our plain CMake package.
...
To forward our package further down the dependency tree, we have to add it as non-catkin dependency via `DEPENDS`. 
...
For example, we can add a ROS node executable `test_node` that depends on the `plain_cmake_lib`.
If the package was found via `find_package`, the library can be linked via `target_link_library`.
Sine we exported the target in the namespace `plain_cmake` we have to link the node against `plain_cmake::plain_cmake_lib`.
